    ...
    <template v-if="formData.bereavment && formData.mentalHealth && formData.physical && formData.other">
        ...
        <div class="card__inner max-w-4xl mx-auto px-8 pb-16">
            ...
            <nav class="pt-5 grid grid-cols-2">
                ...
                <button class="bg-primary outline-primary px-3 py-2 text-white rounded hover:bg-green ml-auto" :class="!formData.clientID || !formData.updateType ? 'inactive': null" @click="$store.commit('currentSectionPlus', null)">NEXT ></button>
            </nav>
        </div>
    </template>
</div>
