...
<div class="card bg-white w-full max-w-7xl mx-auto shadow rounded text-left">
    ...
    <div class="card__inner max-w-4xl mx-auto px-8 py-16">
        ...
        <input v-model="clientID" type="text" placeholder="-- Please enter client ID --" class="block border-b-2 py-2 px-3 text-black">
        <br>
        ...
        <div v-for="(type, i) in typeList" :key="i">
            <label class="inline-flex block items-center">
                <input type="radio" class="form-radio block text-primary form-radio h-4 w-4" name="radio" :value="type" v-model="updateType">
                <span class="ml-2 block">{{type}}</span>
            </label>
        </div>
        <nav class="text-right pt-5">
            <button class="bg-primary outline-primary px-3 py-2 text-white rounded hover:bg-green ml-auto " :class="!clientID || !updateType ? 'inactive': null" @click="$store.commit('currentSectionPlus', null)">COMPLETE</button>
        </nav>
    </div>
</div>
</template>
